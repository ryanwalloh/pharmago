    ...
    try:
        ...
        def authenticate_rider():
            ...
            if not user:
                ...
                return {'error': JsonResponse({'success': False, 'error': 'Invalid credentials'}, status=401)}

            if user.role != User.UserRole.RIDER:
                return {'error': JsonResponse({'success': False, 'error': 'Only rider accounts can login here'}, status=403)}

            if not user.is_active:
                return {'error': JsonResponse({'success': False, 'error': 'Account is deactivated'}, status=401)}

            if user.status != User.UserStatus.ACTIVE:
                return {'error': JsonResponse({'success': False, 'error': f'Account status: {user.get_status_display()}'}, status=401)}

            try:
                rider = Rider.objects.get(user=user)
            except Rider.DoesNotExist:
                return {'error': JsonResponse({'success': False, 'error': 'Rider profile not found'}, status=404)}
            ...
            session_data = {
                ...
                'stats': {
                    ...
                    'success_rate': 100.0 if rider.total_deliveries > 0 else 0.0,
                    'total_earnings': float(rider.total_earnings) if rider.total_earnings else 0.0,
                }
            }
        ...
        result = await authenticate_rider()
        if 'error' in result:
            return result['error']

        return JsonResponse(result['session'], status=200)
...
@csrf_exempt
async def available_orders_count(request):
    """Async count of unassigned orders ready for riders."""
    if request.method != 'GET':
        return JsonResponse({'success': False, 'error': 'Method not allowed'}, status=405)
    ...
    try:
        ...
        logger.info(f"📦 Available orders count: {count}")
        
        return JsonResponse({'success': True, 'count': count})
    except Exception as e:
        logger.error(f"❌ Error fetching available orders count: {str(e)}", exc_info=True)
        return JsonResponse({'success': False, 'error': 'Failed to fetch orders count'}, status=500)
